...
/** Map<sessionToken, { ws, uid, displayName }> */
const connections = new Map();

/** Map<uid, { ws, sessionToken }> for reverse lookup */
...
export function initWebSocket(server) {
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
      ...
      if (msg.type === 'auth') {
        const authUid = authenticateSession(msg.sessionToken);
        if (authUid && handleAuth(ws, msg, authUid)) {
          authed = true;
          uid = authUid;
        }
        return;
      ...
      // Verify session on each message
      const msgUid = authenticateSession(msg.sessionToken);
      if (!msgUid) {
        ws.send(JSON.stringify({ type: 'error', payload: { message: 'Session expired' } }));
        return;
      }

      dispatch(ws, msgUid, msg);
    });
...
function handleAuth(ws, msg, uid) {
  ...
  const existing = playerConnections.get(uid);
  if (existing) {
    connections.delete(existing.sessionToken);
  }

  connections.set(msg.sessionToken, { ws, uid, displayName: player.display_name });
  playerConnections.set(uid, { ws, sessionToken: msg.sessionToken });
...
function dispatch(ws, uid, msg) {
  switch (msg.type) {
    case 'find_match':
      handleFindMatch(ws, uid, msg.payload);
      break;
    case 'create_room':
      handleCreateRoom(ws, uid, msg.payload);
      break;
    case 'join_room':
      handleJoinRoom(ws, uid, msg.payload);
      break;
    case 'submit_move':
      handleSubmitMove(ws, uid, msg);
      break;
    case 'suit_declaration':
      handleSuitDeclaration(ws, uid, msg);
      break;
    case 'setup_choice':
...
function handleFindMatch(ws, uid, payload) {
  ...
  if (result.matched) {
    // Both players matched — notify both
...
function handleCreateRoom(ws, uid, payload) {
  const { gameType } = payload;
  const player = getPlayer(uid);
...
function handleJoinRoom(ws, uid, payload) {
...
function handleSubmitMove(ws, uid, msg) {
  ...
  const result = room.session.submitMove(uid, msg.payload);
...
function handleSuitDeclaration(ws, uid, msg) {
  const room = verifyRoomMembership(ws, uid, msg.roomId);
...
function handleCancelMatch(ws, uid, payload) {
  ...
  // Only allow canceling waiting rooms the player is in
...
function startGame(room) {
  // Dynamic import of game session based on type
...
function broadcastToRoom(room, event) {
  // If event targets a specific player (e.g., Crazy 8 personalized state)
  if (event.targetUid) {
  ...
  for (const p of room.players) {
    ...
    // For games with private state (crazy8), send personalized views
...
function handleDisconnect(uid) {
  ...
  if (conn) {
    connections.delete(conn.sessionToken);
  }
  playerConnections.delete(uid);

  // Find any room this player is in
...
function handleForfeit(room, disconnectedUid) {
  ...
  // Clean up the room after a short delay to let messages drain
