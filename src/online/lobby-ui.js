...
import { getSessionToken, getPlayer, clearSession } from './session.js';
...
export function createLobbyUI(container, callbacks) {
  ...
  function show() {
    ...
    overlay.querySelector('#lobby-logout-btn').addEventListener('click', () => {
      clearSession();
      hide();
      callbacks.onLogout();
    });
  ...
  function handleAutoMatch() {
    ...
    wsClient.send({ type: 'find_match', payload: { gameType: selectedGameType } });
    showWaiting('Finding opponent...', null);
  }

  function handleCreateRoom() {
    if (!wsClient || !wsClient.isConnected()) return;
    wsClient.send({ type: 'create_room', payload: { gameType: selectedGameType } });
  ...
  function handleJoinRoom() {
    ...
    if (!wsClient || !wsClient.isConnected()) return;
    wsClient.send({ type: 'join_room', payload: { roomCode: code } });
    showWaiting('Joining room...', null);
  }

  function cancelMatch() {
    if (currentRoomId && wsClient && wsClient.isConnected()) {
      wsClient.send({ type: 'cancel_match', payload: { roomId: currentRoomId } });
    }
    currentRoomId = null;
    showGameSelect();
  }
